#!/usr/bin/env tsx
/**
 * Recent Email Backfill Script
 *
 * Fetches and processes financial emails from the last 2 days
 *
 * Usage:
 *   cd ops
 *   npx tsx backfill-recent.ts
 */
...
class RecentBackfill {
  ...
  private async listMessages(): Promise<string[]> {
    console.log('📬 Fetching ALL Yappy message IDs...\n');

    const query = `from:@yappy.com.pa OR from:@bancogeneral.com label:financial`;
  ...
  async run(): Promise<void> {
    console.log('\n🚀 Starting Recent Emails Backfill\n');
...
// Main execution
const backfill = new RecentBackfill();
